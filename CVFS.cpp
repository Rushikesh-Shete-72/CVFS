...
typedef struct filetable
{
  ...
  int mode;// 1 2 3
  PINODE ptrinode;
}FILETABLE,*PFILETEBLE;

typedef  struct ufdt
{
  PFILETEBLE ptrfiletable;
}UFDT;
UFDT UFDTArr[10];
...
void man(char *name)
{
  ...
  {
    printf("Description: Used to delet the  file\n");
  ...
  void DisplayHelp()
  {
     ...
     printf("closeall: To close all opened fils\n");
     printf("read: TO Read the contents into file\n");
     printf("write: To Write contents into file\n");
     printf("exit: To Terminate contents into file system\n");
     printf("stat: To Display information of file using name\n");
     printf("fstat: To Displayinformation of file using file discriptor\n");
 ...
 int CreateFile(char *name,int permission)
{
  ...
  UFDTArr[i].ptrfiletable=(PFILETEBLE)malloc(sizeof(FILETABLE));
...
InitialiseSuperBLock();
 ...
 while(1)
 {
   ...
   if(count==1)
  {
    ...
    else if(strcmp(command[0],"closesell")==0)
    {
      CloseAllFile();
  ...
  else if(count==3)
  {
       if (strcmp(command[0],"create")==0)
       {
        ...
        //ret=CreateFile(Demo.txt,3)
         ...
         if(ret>=0)
            printf("File is successfuly created with file descriptor: %d\n",ret);
          if (ret==-1)
              printf("ERROR: Incorrect parameters\n");
          if(ret==-2)
      ...
      {
        ret=OpenFile(command[0],atoi(command[2]));
        if(ret>=0)
          printf("File is successfuly opened with file descriptor: %d\n",ret);
        if(ret==-1)
            printf("ERROR:Incorrect parameters\n");
        if(ret==-2)
      ...
      else if (strcmp(command[0],"read")==0)
      {
        ...
        if(ptr==NULL)
        {
             printf("ERROR:Memmory alocation failure\n");
             continue;
        }
        ...
        if(ret==-1)
         ...
         if(ret==-2)
             printf("ERROR2:Permission denied\n");
          if(ret==-3)
